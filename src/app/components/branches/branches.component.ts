...
export class BranchesComponent implements OnInit {
  ...
  getBranches(): void {
    this.http
      .get<Branch[]>('http://localhost:5298/api/Branch')
      .subscribe((data) => {
  ...
  toggleStatus(branchId: number): void {
    ...
    if (branch) {
      ...
      this.http
        .put(`http://localhost:5298/api/Branch/${branchId}`, branch)
        .subscribe((response) => {
  ...
  saveBranch(branch: Branch): void {
    this.http
      .put(`http://localhost:5298/api/Branch/${branch.id}`, branch)
      .subscribe(() => {
  ...
  deleteBranch(branchId: number): void {
    this.http
      .delete(`http://localhost:5298/api/Branch/${branchId}`)
      .subscribe(() => {
